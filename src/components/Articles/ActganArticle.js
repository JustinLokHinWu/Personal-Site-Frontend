  ...
  return (
    <div>
        ...
        <Paragraph>
          ...
          on AC-GAN (Auxillary Classifier Generative Adversarial Network) by using a
          transformer-based architecture for the "discriminator" network. ACTGAN was designed and
          implemented with a partner as a final project for a machine learning course.
        </Paragraph>
        <Paragraph>Several models trained on a variety of datasets are available for demo. The user
          can generate images by specifying the dataset of the model, the epoch (early epochs are
          from the beginning of training, while later epochs are from the end of training), the
          class of image, and an optional seed.
        </Paragraph>
        <Divider />
        <ModelDemo
            info={info}
